...
def parse_adblock_lines(lines: List[str]) -> List[str]:
    ...
    for raw in lines:
        ...
        if '/' not in line and '^' not in line and ' ' not in line and not line.startswith('/'):
            token = line
...
def cf_post_batch(account_id: str, list_id: str, headers: Dict[str, str], items: List[str]):
    url = f"https://api.cloudflare.com/client/v4/accounts/{account_id}/gateway/lists/{list_id}/items"
...
def push_combined(account_id: str, list_id: str, headers: Dict[str, str], all_domains: List[str]):
...
def push_split(account_id: str, headers: Dict[str, str], category_map: Dict[str, Set[str]], default_list_id: str = None):
    for cat, domains in category_map.items():
        env_name = f"LIST_ID_{cat.upper()}"
        list_id = os.getenv(env_name) or default_list_id
        if not list_id:
            print(f"[WARN] No LIST_ID provided for category '{cat}' (env {env_name} or default). Skipping")
            continue
        domain_list = sorted(domains)
        print(f"[INFO] Pushing {len(domain_list)} domains to list {list_id} (category {cat})")
        d = cf_delete_all_items(account_id, list_id, headers)
        print(f"  delete status: {d.status_code}")
...
def main():
    ...
    p.add_argument('--split', action='store_true', help='Push per-category lists. Requires LIST_ID_<CATEGORY> env vars or default LIST_ID')
    args = p.parse_args()
